...
```{r}
#H_array = c(3,11,19,27,35,43,51,59,67,75,83)
...
```{r}
smooth_list1_aus = super_smoother(time_idx1_aus, high1_aus, dnorm, H_array,  error_estimate="loocv", error_bdw=H_array[2])
```
...
```{r}
smooth_list1_sd = super_smoother(time_idx1_sd, high1_sd, dnorm, H_array, error_estimate="loocv")
```
...
```{r}
gg = ggplot(plot_df_trunc) + geom_line(aes(x=Date, y=Daily_High_Temp), color="gray") + geom_line(aes(x=Date, y=Y_hat), color="red") + facet_wrap( ~ City, scales="free") + labs(title="Daily High Temperatures with LOOCV Super-Smooth", x="Date", y="High Temp. (F)") + theme(plot.title = element_text(hjust = 0.5))
print(gg)
```
