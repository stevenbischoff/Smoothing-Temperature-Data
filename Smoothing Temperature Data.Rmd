...
I wanted to investigate temperature smoothing across a wide variety of climates, so I requested data from 4 different cities. Table 1 provides a summary: 

```{r fig.cap="Data source descriptions"}
...
To make computation more manageable, I truncated each data set to 1993 - 2022. Figures 2 and 3 display time series and box plots of the truncated data sets.  
...
The generally higher bandwidths lead to much-improved smooths which can be seen in Figure 13. The smooths are much less wiggly than the LOOCV super-smooths in Figure 12 (Section 6.1), doing a better job of capturing broader seasonal trends instead of short-term deviations. For instance, the LOOCV smooth for Austin in Winter 1995-1996 contains 3 prominent local minima and is yanked upwards by the February 1996 heat wave. The KBCV smooth, on the other hand, contains only a single minimum in that period and is less sensitive to the heat wave.

Figure 13 also displays the bandwidths selected by the super-smoother at each point (along with the $h^*=15$ smooth applied to them). We can see some seasonality in the bandwidth selections that match the advertised properties of the super-smoother. Higher bandwidths tend to be selected in higher-variance periods (e.g. Austin winters, Baltimore springs) and lower bandwidths tend to be selected in lower-variance periods (e.g. Austin summers).
