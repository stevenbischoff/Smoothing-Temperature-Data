...
The data used in this project comes from the National Oceanic and Atmospheric Administration (NOAA) [1].  I used the search tool to find stations that have had 100% Daily Summary coverage for many decades.  

I wanted to investigate temperature smoothing across a wide variety of climates, so I requested data from 4 different cities. The following table provides a summary: 
...
kable(table_df, col.names=c("City", "Station ID", "First Available"))
```

To make computation more manageable, I truncated each data set to 1993 - 2022. Below are time series and box plots of the truncated data sets.  
...
```{r}
par(mar=c(5,5,4,2))
boxplot(Daily_High_Temp ~ City, plot_df, cex.lab=1, cex.axis=1, main="Daily Highs Boxplot by City", ylab="High Temp. (F)")
```
...
gg = ggplot(plot_df_trunc) + 
  ...
  labs(title="Daily High Temperatures with Normal Kernel Smooth (h=14)", 
       x="Date", y="High Temp. (F)") + 
...
gg = ggplot(plot_df_loocv, aes(x=h, y=MSE)) + 
  geom_line() + 
  facet_wrap( ~ City, scales="free") + 
  labs(title="LOOCV MSE by bandwidth", 
       y="Mean Squared Error (MSE)", x="Bandwidth") + 
  theme(plot.title = element_text(hjust = 0.5))
print(gg)
```
...
The first figure below from [2] shows an example non-constant variance data set with a super-smoother fit given 3 candidate bandwidths (0.05, 0.2, 0.5). The second figure shows the optimal bandwidth selected by LOOCV at each point as well as the smooth applied to these bandwidths. Note that higher bandwidths are selected in higher-variance regions, allowing the smooth to cut through the noise, while lower bandwidths are selected in lower-variance regions, allowing the smooth to better capture the trend.
...
gg = ggplot(plot_df_trunc) + 
  ...
  geom_line(aes(x=Date, y=Y_hat), color="red") + 
  facet_wrap( ~ City, scales="free") + 
  labs(title="Daily High Temperatures with LOOCV Super-Smooth (h*=15)", 
       x="Date", y="High Temp. (F)") + 
  theme(plot.title = element_text(hjust = 0.5))
print(gg)
```
...
*K*-block cross validation is a LOOCV alternative designed to help with this problem.[4] 

```{r}
smooth_list1_aus = super_smoother(time_idx1_aus, high1_aus, dnorm, H_array,
                                  error_estimate="kbcv", kb_window=3,
                                  error_bdw=H_array[2]) # 7,19
```
...
data1_aus$Y_hat = Y_hat1_aus
data1_aus$h_min = h_min1_aus
data1_aus$smoothed_h_min = smoothed_h_min1_aus
```

```{r}
mean(smoothed_h_min1_aus)
```
...
data1_sd$Y_hat = Y_hat1_sd
data1_sd$h_min = h_min1_sd
data1_sd$smoothed_h_min = smoothed_h_min1_sd
```

```{r}
mean(smoothed_h_min1_sd)
```

```{r}
plot_df_temp = data.frame(TMAX=c(data1_aus$TMAX, data1_sd$TMAX),
                     DATE=c(data1_aus$DATE, data1_sd$DATE),
                     Y_hat = c(data1_aus$Y_hat, data1_sd$Y_hat),
                     h_min = c(data1_aus$h_min, data1_sd$h_min),
                     smoothed_h_min = c(data1_aus$smoothed_h_min,
                                        data1_sd$smoothed_h_min),
                     City=c(rep("Austin", n1_aus),
                            rep("S.D.", n1_sd)))
```

```{r}
plot_df_temp_trunc = plot_df_temp[(plot_df_temp$DATE >= "1995-10-01") & (
  plot_df_temp$DATE <= "1999-09-30"),]

gg1 = ggplot(plot_df_temp_trunc) + 
  geom_line(aes(x=DATE, y=TMAX), color="gray") + 
  geom_line(aes(x=DATE, y=Y_hat), color="red") +  
  facet_wrap( ~ City, scale="free") + 
  labs(title="Daily High Temperatures with KBCV Super-Smooth (h*=15, k=3)", 
       x="", y="High Temp. (F)") + 
  theme(plot.title = element_text(hjust = 0.5))

gg2 = ggplot(plot_df_temp_trunc) + 
  geom_point(aes(x=DATE, y=h_min), color="black") + 
  geom_line(aes(x=DATE, y=smoothed_h_min), color="gray") +  
  facet_wrap( ~ City) + 
  labs(title="KBCV Super-Smoother (h*=15, k=3) Selected Bandwidths", 
       x="Date", y="Selected Bandwidth") + 
  theme(plot.title = element_text(hjust = 0.5))
...
data1_mn$Y_hat = Y_hat1_mn
data1_mn$h_min = h_min1_mn
data1_mn$smoothed_h_min = smoothed_h_min1_mn
```

```{r}
mean(smoothed_h_min1_mn)
```
...
data1_bt$Y_hat = Y_hat1_bt
data1_bt$h_min = h_min1_bt
data1_bt$smoothed_h_min = smoothed_h_min1_bt
```

```{r}
mean(smoothed_h_min1_bt)
```

```{r}
plot_df_temp = data.frame(TMAX=c(data1_mn$TMAX, data1_bt$TMAX),
                     DATE=c(data1_mn$DATE, data1_bt$DATE),
                     Y_hat = c(data1_mn$Y_hat, data1_bt$Y_hat),
                     h_min = c(data1_mn$h_min, data1_bt$h_min),
                     smoothed_h_min = c(data1_mn$smoothed_h_min,
                                        data1_bt$smoothed_h_min),
